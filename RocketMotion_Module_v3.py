...
class SSTOAscent:
    ...
    # CHECK WHETHER MODULES ARE PROPERLY INTERGRATED
    ...
    def __init__(self, Ascent_vehicle: str,
        ...
        self.orbit = False
        self.Target_velocity = np.sqrt(GRAVITATIONAL_CONSTANT*self.bodies.get_body("Earth").mass/(spice_interface.get_average_radius("Earth")+self.Target_Altitude))
...
### ACCELERATION FORCES FOR THE ROCKET MOTION
    ...
    def set_thrust_model(self, ThrustMagnitude, Thrustguidance, ThrustISP , Thrust_class : classmethod):
...
### TERMINATION CONDITIONS FOR THE ROCKET MOTION MOUDEL
    #TODO | STATUS : COMPLETED
    def set_terminationConditions(self):
...
### INITIAL CONDITIONS FOR THE ROCKET MOTION MODULE
    #TODO | STATUS : COMPLETED (COULD BE IMPROVED)
...
### OPTIMISATION PROBLEM
    #TODO | STATUS : UNCOMPLETED (need to be modified before it can be used by pygmo)
    def fitness(self, x: float):
        if self.Status_setup == None:
            self.get_acceleration_settings()
            self.get_mass_rate_settings()
            self.get_propagation_model()
            self.Status_setup = "has been done"

        self.ThrustClass.set_turn_rate(x)

        #TODO : the intergrator settings should be reevaluated to find the best method API website
        intergrator_settings = propagation_setup.integrator.runge_kutta_4(
            self.StartEpoch,
            self.dt
        )
        dynamic_simulator = SingleArcSimulator(
            self.bodies,
            intergrator_settings,
            self.Propagation_settings
        )
